 ...
 *
 * Tests all builder features against a real SQLite in-memory database with rich test data.
 * From simple queries to complex multi-table JOINs, CTEs, subqueries, and JSON.
 *
 * SQLite-specific notes:
 * - Uses :memory: — connection is static/singleton, data persists across tests
 * - No FULL JOIN / FULL OUTER JOIN support
 * - No JOIN in UPDATE/DELETE
 * - No ORDER BY/LIMIT in UPDATE/DELETE
 * - No ON DUPLICATE KEY UPDATE — use orIgnore() or replace() or onConflict()->doUpdate()
 * - JSON: json_extract() / json_each() (SQLite 3.38+)
 * - Window functions: SQLite 3.25+
 * - CTEs: SQLite 3.8.3+
 */
class ComprehensiveSqliteTest extends TestCase
{
    private const DIALECT = 'sqlite';
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo = $this->db->getSqliteConnection();
    ...
    {
        // SQLite: HAVING with prepared statements requires non-prepared mode for reliable HAVING
        $query = (new DbQuery())->select('status, COUNT(*) as cnt')->from('users')
            ->groupBy('status')
    ...
    public function testGroupByMultipleColumnsWithHaving(): void
    {
        ...
        // SQLite: HAVING with prepared statements — use non-prepared mode
        $query = (new DbQuery())
    ...
    {
        // users -> orders filtered by status
        $results = $this->fetch(
            (new DbQuery())
                ->select('u.name, o.product, o.amount')
                ->from('users', 'u')
    ...
    /**
     * SQLite does not support FULL JOIN — builder throws InvalidArgumentException
     */
    public function testFullJoinThrowsException(): void
    {
        ...
        $query = (new DbQuery())
            ...
            ->from('users', 'u')
            ->fullJoin('orders', 'u.id = o.user_id', 'o');

        $query->sql(self::DIALECT);
    }
    ...
    public function testSubqueryInFrom(): void
    {
        ...
        // SQLite: WHERE with prepared statements on derived tables — use non-prepared mode
        $subquery = (new DbQuery())
    ...
    {
        // SQLite: json_array_length with prepared statements — use non-prepared mode
        $query = (new DbQuery())
            ->select('name')
            ->from('users')
    ...
    public function testComplexSubqueryWithJoinAndBrackets(): void
    {
        ...
        // Use non-prepared mode to avoid SQLite quirks with complex nested subqueries
    ...
    {
        // SQLite does not support CREATE TABLE LIKE — use CREATE TABLE AS SELECT WHERE 0
        $this->pdo->exec(
            "CREATE TABLE archived_users AS SELECT * FROM users WHERE 0"
        );
    ...
    public function testInsertOrIgnoreOnConflict(): void
    {
        // email 'john@example.com' already exists — OR IGNORE should silently skip
        $insert = new DbInsert();
        $prepared = $insert
            ->into('users')
            ->orIgnore()
            ->fields('name', 'email', 'status', 'age')
            ->values('Duplicate John', 'john@example.com', 'active', 55)
            ->sql(self::DIALECT);
        ...
        $row = $this->fetchOne(
            (new DbQuery())->select('name, age')->from('users')->where('email')->equals('john@example.com')
        );
    ...
    public function testInsertReplaceOnConflict(): void
    {
        // email 'john@example.com' already exists — REPLACE deletes old row and inserts new one
        $insert = new DbInsert();
        $prepared = $insert
            ->replace()
            ->into('users')
            ->fields('name', 'email', 'status', 'age')
            ->values('John Replaced', 'john@example.com', 'inactive', 45)
            ->sql(self::DIALECT);

        $stmt = $this->pdo->prepare($prepared->sql());
        $stmt->execute($prepared->bindings());

        // Should still be 6 rows total
        $this->assertEquals(6, $this->db->countRows($this->pdo, 'users'));

        // New data should be present
        $row = $this->fetchOne(
            (new DbQuery())->select('name, age, status')->from('users')->where('email')->equals('john@example.com')
        );

        $this->assertNotFalse($row);
        $this->assertEquals('John Replaced', $row['name']);
        $this->assertEquals(45, (int) $row['age']);
        $this->assertEquals('inactive', $row['status']);
    }

    public function testInsertOnConflictDoUpdate(): void
    {
        // email 'john@example.com' already exists — ON CONFLICT DO UPDATE
        $insert = new DbInsert();
        $prepared = $insert
            ->into('users')
            ->fields('name', 'email', 'status', 'age')
            ...
            ->onConflict('email')
            ->doUpdate(['name' => 'Updated John', 'status' => 'vip', 'age' => 31])
            ->sql(self::DIALECT);
    ...
    {
        // email 'john@example.com' already exists — ON CONFLICT DO NOTHING
        $insert = new DbInsert();
        $prepared = $insert
            ->into('users')
            ->fields('name', 'email', 'status', 'age')
            ->values('Should Not Insert', 'john@example.com', 'active', 99)
            ->onConflict('email')
            ->doNothing()
            ->sql(self::DIALECT);
    ...
    public function testDeleteWithExists(): void
    {
        ...
        // SQLite: EXISTS with correlated subquery and prepared bindings — use non-prepared mode
        $subquery = (new DbQuery())
            ->select('1')
            ->from('users')
