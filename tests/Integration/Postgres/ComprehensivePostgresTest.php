 ...
 *
 * Tests all builder features against a real PostgreSQL database with rich test data.
 * From simple queries to complex multi-table JOINs, CTEs, subqueries, and JSON.
 *
 * Differences from MySQL:
 * - FULL OUTER JOIN supported and tested
 * - No JOIN in UPDATE/DELETE (PostgreSQL uses FROM clause — not supported by builder)
 * - No ORDER BY/LIMIT in UPDATE/DELETE (PostgreSQL does not support it)
 * - No ON DUPLICATE KEY UPDATE — uses ON CONFLICT ... DO UPDATE / DO NOTHING instead
 * - Boolean values: PostgreSQL returns true/false strings
 * - Identifier quoting uses " instead of `
 * - JSON uses JSONB type with ->>/-> operators (abstracted by builder)
 */
class ComprehensivePostgresTest extends TestCase
{
    private const DIALECT = 'postgres';
    ...
    protected function setUp(): void
    {
        ...
        $this->pdo = $this->db->getPostgresConnection();
    ...
    public function testFullJoin(): void
    {
        ...
        // All users and all orders, even if there is no match on either side
        $results = $this->fetch(
            (new DbQuery())
        ...
        // 6 users, 8 orders; Charlie has no orders → appears with NULL product
        // All 8 orders appear (some users appear multiple times)
        $this->assertGreaterThanOrEqual(8, count($results));
    }

    public function testMultipleJoins(): void
    {
        // users -> orders filtered by conditions
        $results = $this->fetch(
            (new DbQuery())
                ->select('u.name, o.product, o.amount')
                ->from('users', 'u')
    ...
    {
        // Create a temp table for archived users (PostgreSQL syntax)
        // INCLUDING ALL copies sequences, constraints, defaults etc.
        $this->pdo->exec('DROP TABLE IF EXISTS archived_users');
        $this->pdo->exec('CREATE TABLE archived_users (LIKE users INCLUDING ALL)');
    ...
    {
        // PostgreSQL: INSERT ... ON CONFLICT (email) DO NOTHING
        // Try to insert a duplicate email — should be silently ignored
        $insert = new DbInsert();
        $prepared = $insert
            ->into('users')
            ->fields('name', 'email', 'status', 'age')
            ->values('Duplicate John', 'john@example.com', 'active', 30)
            ->onConflict('email')
            ->doNothing()
            ->sql(self::DIALECT);
        ...
        $row = $this->fetchOne(
            (new DbQuery())->select('name')->from('users')->where('email')->equals('john@example.com')
        );

        $this->assertNotFalse($row);
        $this->assertEquals('John Doe', $row['name']); // original name preserved

        // Total user count must not have increased
        $this->assertCount(6, $this->fetch((new DbQuery())->select('*')->from('users')));
    }
